...
diferencias = variedad2 - variedad1
...
```{r}
marca_A = c(34400, 45500, 36700, 32700, 32000, 38700, 38100, 31100)
marca_B = c(36700, 46800, 37700, 31100, 31700, 37400, 38900, 31500)

diferencias = marca_A - marca_B
diferencias
...
t_critico
margen_error = t_critico * sd_diferencias / sqrt(n)
margen_error
...
```{r}
# Datos de los tiempos de secado en horas
pintura_A = c(3.5, 2.7, 3.9, 4.2, 3.6, 4.4, 3.1, 3.0, 4.0, 3.6, 2.7, 3.3, 5.2, 4.2, 4.4)
pintura_B = c(4.7, 3.9, 4.5, 5.5, 4.0, 5.3, 4.9, 4.6, 5.2, 3.7, 4.4, 4.8, 5.1, 5.4, 4.8)

# Tamaño de la muestra
n_A = length(pintura_A)
n_B = length(pintura_B)

media_A = mean(pintura_A)
media_A
media_B = mean(pintura_B)
media_B
var_A = var(pintura_A)
var_A
var_B = var(pintura_B)
var_B
var_comb = ((n_A - 1) * var_A + (n_B - 1) * var_B) / (n_A + n_B - 2)
var_comb
confianza = 0.95
confianza
gl = n_A + n_B - 2
...
t_critico = qt((1 + confianza) / 2, gl)
t_critico
error_estandar = sqrt(var_comb * (1/n_A + 1/n_B))
error_estandar
...
F_critico_inferior = qf((1 - confianza) / 2, gl_A, gl_B)
F_critico_inferior
F_critico_superior = qf((1 + confianza) / 2, gl_A, gl_B)
F_critico_superior
limite_inferior = var_A / var_B / F_critico_superior
limite_inferior
limite_superior = var_A / var_B / F_critico_inferior
limite_superior
